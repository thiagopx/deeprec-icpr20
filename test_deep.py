...
records = []
for docs, dataset in zip([group1, group2], ['isri-ocr', 'cdip']):
    # algorithm
    train_dir = '{}/pretrain/{}/1'.format(CONFIG_TRAIN_BASE_DIR, dataset)
    ...
    algorithm = Proposed(
        ...
        verbose=False, representation=CONFIG_TEST_REPRESENTATION, thresh='sauvola'
    )
    for doc in docs:
        print('[{:.2f}%] doc={} train dataset={} :: '.format(100 * processed / total, doc, dataset), end='')
        processed += 1
        ...
        records.append(['deep', doc, acc, init_permutation, solution, compatibilities.tolist()])

os.makedirs('results', exist_ok=True)
json.dump(records, open('results/deep.json', 'w'))
